...
function Tarjeta({ icono, titulo, texto }) {
  return (
    <div className="flex min-h-0 flex-col gap-1.5 rounded-2xl border border-tierra-200 bg-white p-3">
      <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-bosque-100 text-bosque-700">
        <Icon name={icono} className="h-5 w-5" />
      </span>
      <h3 className="text-sm font-bold leading-tight text-slate-800">{titulo}</h3>
      <p className="text-xs leading-tight text-slate-600">{texto}</p>
    </div>
  )
}
  ...
  const slides = [
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col items-center justify-center gap-4 text-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-3xl bg-agua-100 text-agua-700">
            <Icon name="agua" className="h-8 w-8" />
          </span>
          <h2 className="text-[clamp(1.5rem,4vw,2.5rem)] font-black text-agua-900">
          ...
          <div className="grid w-full max-w-4xl grid-cols-1 gap-2 sm:grid-cols-3">
            {IDEAS.map((c) => (
    ...
    },
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col gap-3">
          <h2 className="shrink-0 text-center text-[clamp(1.25rem,3vw,2rem)] font-black text-agua-900">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 content-center gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {PASOS.map((p, i) => (
              <div key={p.titulo} className="flex min-h-0 flex-col gap-1.5 rounded-2xl border border-tierra-200 bg-white p-3">
                <div className="flex items-center gap-2">
                  <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-agua-100 text-agua-700">
                ...
                <p className="text-xs leading-tight text-slate-600">{p.texto}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col gap-3">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 content-center gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {categories.map((c) => (
              <div key={c.categoria} className="flex min-h-0 items-start gap-2 rounded-2xl bg-tierra-50 p-2.5">
