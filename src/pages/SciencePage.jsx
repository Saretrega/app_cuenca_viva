  ...
  const slides = [
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col gap-3">
          <header className="shrink-0 text-center">
            <h2 className="text-[clamp(1.25rem,3vw,2rem)] font-black text-agua-900">Escala ordinal de efecto</h2>
            <p className="mx-auto mt-1 max-w-3xl text-sm text-slate-700">
              Cada valor entre -3 y +3 expresa la dirección e intensidad esperada de un efecto, no una
              medición física.
            </p>
          </header>
          <div className="grid min-h-0 flex-1 grid-cols-1 content-center gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {RANGOS_EFECTO.map((r) => (
              <div key={r.valor} className="flex items-center gap-3 rounded-2xl border border-tierra-200 bg-white p-3">
                <span className={`flex h-10 w-12 shrink-0 items-center justify-center rounded-xl font-bold tabular-nums ${COLOR_ESCALA(r.valor)}`}>
                  {r.valor > 0 ? '+' : ''}
                  {r.valor}
                </span>
    ...
    },
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col items-center justify-center gap-4">
          <h2 className="text-center text-[clamp(1.25rem,3vw,2rem)] font-black text-agua-900">
            Doce efectos por decisión
          </h2>
          <p className="max-w-3xl text-center text-sm text-slate-700">
          ...
          <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-tierra-200 bg-white">
            <div className="grid grid-cols-[1.4fr_repeat(3,1fr)] bg-tierra-50 text-xs font-semibold text-slate-600">
              <span className="px-3 py-2">Indicador</span>
              {TRAMOS.map((t) => (
                <span key={t} className="px-3 py-2 text-center">
                  {TRAMO_LABEL[t]}
                </span>
              ))}
            </div>
            {INDICADORES.map((ind) => (
              <div key={ind} className="grid grid-cols-[1.4fr_repeat(3,1fr)] border-t border-tierra-100 text-xs">
                <span className="flex items-center gap-1.5 px-3 py-2 text-slate-700">
                  <Icon name={INDICADOR_ICONO[ind]} className="h-4 w-4 text-agua-600" />
                  {INDICADOR_CORTO[ind]}
                </span>
                {TRAMOS.map((t) => (
                  <span key={t} className="px-3 py-2 text-center font-mono text-slate-400">
                    -3…+3
                  </span>
                ))}
              </div>
    ...
    },
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 items-center justify-center">
          <div className="max-w-3xl rounded-3xl border-2 border-alerta-400/60 bg-alerta-400/10 p-6 text-center">
            <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-3xl bg-alerta-400/30 text-alerta-600">
              <Icon name="alerta" className="h-7 w-7" />
            </span>
            <h2 className="mt-3 text-[clamp(1.25rem,3vw,1.75rem)] font-black text-alerta-600">
              Aviso importante
            </h2>
    ...
    },
    {
      ...
      contenido: (
        <div className="flex h-full min-h-0 flex-col gap-3">
          <header className="shrink-0 text-center">
            <h2 className="text-[clamp(1.25rem,3vw,2rem)] font-black text-agua-900">Fuentes y referencias</h2>
            <p className="mx-auto mt-1 max-w-3xl text-xs text-slate-600 sm:text-sm">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 content-start gap-1.5 overflow-hidden sm:grid-cols-2 xl:grid-cols-3">
            {sources.map((s) => (
